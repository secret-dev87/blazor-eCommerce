namespace EcommerceBlazor.Server.Services.ProductService
{
    public class ProductService : IProductService
    {
        ...
        public async Task<ServiceResponse<Product>> GetProductAsync(int productId)
        {
            ...
            if (_httpContextAccessor.HttpContext.User.IsInRole("Admin")) //for admin
            {
            ...
            else
            {
                ...
                .Where(v => v.Visible && !v.Deleted)) //filter variants to show only Visible & not Deleted
                .ThenInclude(v => v.ProductType) //using variants it loads types
                .FirstOrDefaultAsync(p => p.Id == productId //returns product by given Id
                    && p.Visible && !p.Deleted); //if it's Visible and not Deleted
            }

            if (product == null)
            {
                //The reason in creating DTO was in
                //The ability to rapidly configure
                //responses from the server and provide
                //extra info as a Message
        ...
        public async Task<ServiceResponse<List<Product>>> GetProductsByCategory(string categoryUrl)
        {
            var response = new ServiceResponse<List<Product>>
            {
                ...
                p.Visible && !p.Deleted)
                .Include(p => p.Variants //only variants for categories
